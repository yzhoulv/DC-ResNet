...
class MyLoss(nn.Module):
    def __init__(self, in_features, out_features, s=30.0, m=0.30, easy_margin=True):
        ...
        self.label = torch.randint(low=0, high=opt.num_classes, size=[1], device="cuda").long()

    def _weight_loss(self, weight):
        label = self.label
    ...
    def forward(self, input):
        ...
        output *= self.s
        # print(output)
        # weight_loss = self._weight_loss(self.weight)
        # weight_loss = 0
...
class AttentionDrop(nn.Module):
    ...
    def forward(self, x):
        ...
        if len(x_con_sq.shape) == 2:
            # print("shape == 2")
            x_con_sq = x_con_sq.unsqueeze(0)
        max_val = torch.max(x_con_sq)
        min_val = torch.min(x_con_sq)
        ...
        threholds = threholds.expand(threholds.shape[0], 64)
        ...
        final_map = mask * x_rand * x_con + x_iptc * (1.0 - x_rand)
...
class ResNetFace(nn.Module):
    def __init__(self, block, layers, use_se=True):
        ...
        self.dropout = AttentionDrop()
        self.fc5 = nn.Linear(512 * 8 * 8, 512)
        self.bn5 = nn.BatchNorm1d(512)
        self.fc6 = MyLoss(512, opt.num_classes, s=30.0, m=0.00, easy_margin=True)
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        feat = self.fc5(x)
        # feat, cal_score = self.dropout(x)
        feat = self.bn5(feat)
        cal_score = self.fc6(feat)

        return feat, cal_score
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.fc5(x)
        feat = self.bn5(x)
        # cls_score, weight_loss = self.fc6(feat, label)
        # cls_score = self.fc6(feat, label)
        cls_score = self.fc6(feat)
        # return feat, cls_score, weight_loss
        return cls_score
...
def resnet34(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load("./output/Res34_20.pth"))
    return model
