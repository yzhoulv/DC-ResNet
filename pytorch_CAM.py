...
from models.resnet import resnet_face18 # resnet_face18
from torch.nn.parameter import Parameter

i = 0  ##testing in what
resnet = resnet_face18(pretrained=True)  # 这里单独加载一个包含全连接层的resnet50模型
image = []
...
class GuidedBackpropReLUModel:
    ...
    def bp_relu(self, module, grad_in, grad_out):
        if isinstance(module, nn.PReLU):
            return torch.clamp(grad_in[0], min=0.0), Parameter(torch.Tensor(1).fill_(0)).cuda()
...
if __name__ == '__main__':
    ...
    model = resnet_face18(pretrained=True)  # 这里相对vgg19而言我们处理的不一样，这里需要删除fc层，因为后面model用到的时候会用不到fc层，只查到fc层之前的所有层数。
    del model.fc
    print(model)
    ...
    for img in image:
        ...
        gb_model = GuidedBackpropReLUModel(model=resnet_face18(pretrained=True), use_cuda=args.use_cuda)
